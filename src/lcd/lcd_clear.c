...
static void LCDwriteCommandSPI(uint32_t data)
{
    CS(0);
    A0(0);
    SPITransmitBlocking(data, 0, 1);
    A0(1);
    CS(1);
}
static void LCDwriteData8SPI(uint32_t data)
{
    CS(0);
    SPITransmitBlocking(data, 0, 1);
    CS(1);
}
static void LCDwriteData16SPI(uint32_t data)
{
    CS(0);
    SPITransmitBlocking(data, 0, 2);
    CS(1);
}
void LCDsetRectangleSPI(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
  LCDwriteCommandSPI(0x2A);
  LCDwriteData16SPI(x1);
  LCDwriteData16SPI(x2);

  LCDwriteCommandSPI(0x2B);
  LCDwriteData16SPI(y1);
  LCDwriteData16SPI(y2);

  LCDwriteCommandSPI(0x2C);
}
void LCDclearSPI() {
  int i, j;

  LCDsetRectangleSPI(0, 0, LCD_PIXEL_WIDTH - 1, LCD_PIXEL_HEIGHT - 1);
  for (i = 0; i < LCD_PIXEL_WIDTH; ++i) {
    for (j = 0; j < LCD_PIXEL_HEIGHT; ++j) {
      LCDwriteData16SPI(LCD_COLOR_GREEN);
    }
  }

...
static void LCDwrite8DMA(uint32_t data) {
    DMA_transfer_request((uint32_t) &data, DMA_BYTE, 1);  
}

static void LCDwrite16DMA(uint32_t data)
{
    DMA_transfer_request((uint32_t) &data, DMA_HALF_WORD, 1);  
}

static void LCDwrite24DMA(uint32_t data)
{
    DMA_transfer_request((uint32_t) &data, DMA_BYTE, 3);  
}

static void LCDwrite32DMA(uint32_t data)
{
    DMA_transfer_request((uint32_t) &data, DMA_WORD, 1);  
}
...
static void LCDcontrollerConfigureDMA(void) {
  ...
  LCDwrite32DMA(0x0422070A);
  LCDwrite32DMA(0x2E30252A);
  LCDwrite32DMA(0x28262E3A);
  LCDwrite32DMA(0x00010313);
  LCDwriteCommandDMA(0xE1);
  LCDwrite32DMA(0x0416060D);
  LCDwrite32DMA(0x2D262327);
  LCDwrite32DMA(0x27252D3B);
  LCDwrite32DMA(0x00010413);
...
static void LCDdrawCharDMA(unsigned c) {
  ...
  for (i = 0; i < CurrentFont->height; ++i) {
    for (j = 0, w = p[i]; j < CurrentFont->width; ++j, w >>= 1) {
      LCDwrite16DMA(w & 1 ? TextColor : BackColor);
    }
  }
  CS(1);
}
